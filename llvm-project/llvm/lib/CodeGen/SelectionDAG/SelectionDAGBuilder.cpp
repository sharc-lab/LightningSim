...
/// expandPartSelect - Expand a llvm(.fpga(.legacy)).part.select intrinsic.
/// Supports both forward and reverse modes.
static SDValue expandPartSelect(const SDLoc &dl, SDValue Arg, SDValue Lo,
  ...
  ConstantSDNode *LoC, *HiC;
  if ((LoC = dyn_cast<ConstantSDNode>(Lo)) &&
      (HiC = dyn_cast<ConstantSDNode>(Hi))) {
    unsigned LoBit = LoC->getZExtValue();
    unsigned HiBit = HiC->getZExtValue();
    bool Reverse = LoBit > HiBit;

    if (Reverse) {
      Arg = DAG.getNode(ISD::BITREVERSE, dl, VT, Arg);
      LoBit = VTSize - LoBit - 1;
      HiBit = VTSize - HiBit - 1;
    }

    if (LoBit != 0) {
      Arg = DAG.getNode(ISD::SRL, dl, VT, Arg,
                        DAG.getConstant(LoBit, dl, MVT::i32));
    }
    if (HiBit != VTSize - 1) {
      APInt Mask = APInt::getLowBitsSet(VTSize, HiBit - LoBit + 1);
      Arg = DAG.getNode(ISD::AND, dl, VT, Arg,
  ...
  // First, handle reverse mode.
  SDValue Reverse = DAG.getSetCC(dl, MVT::i1, Lo, Hi, ISD::SETGT);
  SDValue Reversed = DAG.getNode(ISD::BITREVERSE, dl, VT, Arg);
  Arg = DAG.getNode(ISD::SELECT, dl, VT, Reverse, Reversed, Arg);

  EVT LoTy = Lo.getValueType();
  SDValue ReverseLo = DAG.getNode(ISD::SUB, dl, LoTy,
                                  DAG.getConstant(VTSize - 1, dl, LoTy), Lo);
  Lo = DAG.getNode(ISD::SELECT, dl, LoTy, Reverse, ReverseLo, Lo);

  EVT HiTy = Hi.getValueType();
  SDValue ReverseHi = DAG.getNode(ISD::SUB, dl, HiTy,
                                  DAG.getConstant(VTSize - 1, dl, HiTy), Hi);
  Hi = DAG.getNode(ISD::SELECT, dl, HiTy, Reverse, ReverseHi, Hi);

  // Common to forward and reverse modes.
  SDValue Width = DAG.getNode(ISD::SUB, dl, MVT::i32, Hi, Lo);
  SDValue Mask = DAG.getNode(ISD::SHL, dl, VT,
                             DAG.getConstant(APInt::getBitsSetFrom(VTSize, 1),
                                             dl, VT),
                             Width);
  Mask = DAG.getNode(ISD::XOR, dl, VT, Mask,
                     DAG.getConstant(APInt::getAllOnesValue(VTSize), dl, VT));
  Arg = DAG.getNode(ISD::SRL, dl, VT, Arg, Lo);
  Arg = DAG.getNode(ISD::AND, dl, VT, Arg, Mask);
  return Arg;
}

/// expandPartSet - Expand a llvm(.fpga(.legacy)).part.set intrinsic.
/// Supports both forward and reverse modes.
static SDValue expandPartSet(const SDLoc &dl, SDValue Arg, SDValue Repl,
                             SDValue Lo, SDValue Hi, SelectionDAG &DAG,
                             const TargetLowering &TLI) {
  EVT VT = Arg.getValueType();
  unsigned VTSize = VT.getScalarSizeInBits();
  EVT ReplVT = Repl.getValueType();
  unsigned ReplVTSize = ReplVT.getScalarSizeInBits();

  // Handle the constant case.
  ConstantSDNode *LoC, *HiC, *ReplC;
  APInt ReplVal;
  if ((LoC = dyn_cast<ConstantSDNode>(Lo)) &&
      (HiC = dyn_cast<ConstantSDNode>(Hi))) {
    unsigned LoBit = LoC->getZExtValue();
    ...
    // Handle the case where the replacement value is a constant.
    if ((ReplC = dyn_cast<ConstantSDNode>(Repl))) {
      ReplVal = ReplC->getAPIntValue();
    }

    if (Reverse) {
      std::swap(LoBit, HiBit);
      unsigned LshrAmt = ReplVTSize - (HiBit - LoBit + 1);
      if (ReplC) {
        ReplVal = ReplVal.reverseBits();
        ReplVal.lshrInPlace(LshrAmt);
      } else {
        Repl = DAG.getNode(ISD::BITREVERSE, dl, ReplVT, Repl);
        if (LshrAmt != 0) {
          Repl = DAG.getNode(ISD::SRL, dl, ReplVT, Repl,
                             DAG.getConstant(LshrAmt, dl, MVT::i32));
        }
      }
    } else if (HiBit != VTSize - 1) {
      APInt ReplMask = APInt::getLowBitsSet(ReplVTSize, HiBit - LoBit + 1);
      if (ReplC) {
        ReplVal &= ReplMask;
      } else {
        Repl = DAG.getNode(ISD::AND, dl, ReplVT, Repl,
                           DAG.getConstant(ReplMask, dl, ReplVT));
      }
    }

    if (ReplC) {
      ReplVal = ReplVal.zextOrTrunc(VTSize);
    } else if (ReplVTSize < VTSize) {
      Repl = DAG.getNode(ISD::ZERO_EXTEND, dl, VT, Repl);
    } else if (ReplVTSize > VTSize) {
      Repl = DAG.getNode(ISD::TRUNCATE, dl, VT, Repl);
    }

    if (LoBit != 0) {
      if (ReplC) {
        ReplVal = ReplVal << LoBit;
      } else {
        Repl = DAG.getNode(ISD::SHL, dl, VT, Repl,
                           DAG.getConstant(LoBit, dl, MVT::i32));
      }
    }

    APInt Mask = ~APInt::getBitsSet(VTSize, LoBit, HiBit + 1);
    Arg = DAG.getNode(ISD::AND, dl, VT, Arg, DAG.getConstant(Mask, dl, VT));
    if (ReplC) {
      Arg = DAG.getNode(ISD::OR, dl, VT, Arg,
                        DAG.getConstant(ReplVal, dl, VT));
    } else {
      Arg = DAG.getNode(ISD::OR, dl, VT, Arg, Repl);
    }
  ...
  EVT LoTy = Lo.getValueType();
  SDValue& Tmp = Lo;
  Lo = DAG.getNode(ISD::SELECT, dl, LoTy, Reverse, Hi, Lo);
  EVT HiTy = Hi.getValueType();
  Hi = DAG.getNode(ISD::SELECT, dl, HiTy, Reverse, Tmp, Hi);
  SDValue Width = DAG.getNode(ISD::SUB, dl, MVT::i32, Hi, Lo);
  SDValue LshrAmt = DAG.getNode(ISD::SUB, dl, MVT::i32,
                                DAG.getConstant(ReplVTSize - 1, dl, MVT::i32),
                                Width);
  SDValue ReverseRepl = DAG.getNode(ISD::BITREVERSE, dl, ReplVT, Repl);
  ReverseRepl = DAG.getNode(ISD::SRL, dl, ReplVT, ReverseRepl, LshrAmt);
  Repl = DAG.getNode(ISD::SELECT, dl, ReplVT, Reverse, ReverseRepl, Repl);

  // Common to forward and reverse modes.
  SDValue ReplMask = DAG.getNode(
    ISD::SHL, dl, ReplVT,
    DAG.getConstant(APInt::getBitsSetFrom(ReplVTSize, 1), dl, ReplVT),
    Width
  );
  ReplMask = DAG.getNode(
    ISD::XOR, dl, ReplVT, ReplMask,
    DAG.getConstant(APInt::getAllOnesValue(ReplVTSize), dl, ReplVT)
  );
  Repl = DAG.getNode(ISD::AND, dl, ReplVT, Repl, ReplMask);
  if (ReplVTSize < VTSize) {
    Repl = DAG.getNode(ISD::ZERO_EXTEND, dl, VT, Repl);
  } else if (ReplVTSize > VTSize) {
    Repl = DAG.getNode(ISD::TRUNCATE, dl, VT, Repl);
  }
  Repl = DAG.getNode(ISD::SHL, dl, VT, Repl, Lo);

  SDValue Mask = ReplMask;
  if (ReplVTSize < VTSize) {
    Mask = DAG.getNode(ISD::ZERO_EXTEND, dl, VT, Mask);
  } else if (ReplVTSize > VTSize) {
    Mask = DAG.getNode(ISD::TRUNCATE, dl, VT, Mask);
  }
  Mask = DAG.getNode(ISD::SHL, dl, VT, Mask, Lo);
  Mask = DAG.getNode(ISD::XOR, dl, VT, Mask,
                     DAG.getConstant(APInt::getAllOnesValue(VTSize), dl, VT));
  Arg = DAG.getNode(ISD::AND, dl, VT, Arg, Mask);
  Arg = DAG.getNode(ISD::OR, dl, VT, Arg, Repl);
...
void SelectionDAGBuilder::visitIntrinsicCall(const CallInst &I,
  ...
  case Intrinsic::part_select:
    setValue(&I, expandPartSelect(sdl, getValue(I.getArgOperand(0)),
                                  getValue(I.getArgOperand(1)),
                                  getValue(I.getArgOperand(2)), DAG, TLI));
    return;
  case Intrinsic::part_set:
    setValue(&I, expandPartSet(sdl, getValue(I.getArgOperand(0)),
                               getValue(I.getArgOperand(1)),
                               getValue(I.getArgOperand(2)),
                               getValue(I.getArgOperand(3)), DAG, TLI));
    return;
  case Intrinsic::sqrt:
  case Intrinsic::fabs:
  case Intrinsic::sin:
